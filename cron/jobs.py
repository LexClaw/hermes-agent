            ...
            if next_run_dt <= now:
                ...
                if kind in {"cron", "interval"} and (now - next_run_dt).total_seconds() > grace:
                    ...
                    if new_next:
                        ...
                        # Persist the fast-forward to storage now. In the built-in
                        # ticker path this is shortly overwritten by advance_next_run
                        # and mark_job_run, but it also protects the crash window and
                        # the external fire_due provider path.
                        for rj in raw_jobs:
                            if rj["id"] == job["id"]:
                                rj["next_run_at"] = new_next
                                needs_save = True
