...
| `realtime` | Same as transcribe PLUS speaks into the meeting via Cartesia TTS. The agent calls `meet_say(text)` and the bot's voice comes out of the call. |

Pick `realtime` only when the user actually wants the agent to speak. It costs real money (Cartesia TTS is pay-per-character) and requires a virtual audio device set up on the machine running the bot.
...
#   Then set CARTESIA_API_KEY and CARTESIA_VOICE_ID (optionally CARTESIA_MODEL_ID) in ~/.hermes/.env
```
...
|- **Barge-in is best-effort.** When a caption arrives attributed to a real participant while the bot is generating audio, the bot sends `response.cancel` to Cartesia. Captions take ~500ms to show up, so the bot will talk over the first second or so of a human interruption.
...
| `audioBytesOut` / `lastAudioOutAt` | How much PCM the Cartesia TTS session has produced. |
...
- `meet_say` text is rate-limited to avoid overwhelming the meeting; spam-protection is the bot's problem, not yours, but still — don't queue hundreds of lines.
